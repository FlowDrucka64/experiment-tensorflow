...
from tasks.util.env import PROJ_ROOT, get_faasm_root, get_example_root

import requests
import time

FAASM_INI_FILE = join(expanduser("~"), ".config", "faasm.ini")
EXAMPLES_DIR = join("/usr", "local", "code", "examples")
TF_EXAMPLES_BUILD_DIR = join(EXAMPLES_DIR, "build", "func", "tf")
...
def fetch_latest_wasm(user, func):
    wasm_path = join(TF_EXAMPLES_BUILD_DIR, func + ".wasm")
    ...
    cp_cmd = "cp {} {}".format(wasm_path, join(exp_wasm_path, func + ".wasm"))
    print(cp_cmd)
